...
from methods.diffusion_cotta import DiffusionCoTTA
...
def evaluate(description):
    ...
    elif cfg.MODEL.ADAPTATION == "diff_cotta":
        model, param_names = setup_diff_cotta(base_model)
    else:
    ...
    if "gradual" in cfg.SETTING and cfg.CORRUPTION.DATASET in {"cifar10_c", "cifar100_c", "imagenet_c", 'cifar10_cd'} and len(cfg.CORRUPTION.SEVERITY) == 1:
        severities = [1, 2, 3, 4, 5, 4, 3, 2, 1]              ###### CHANGED
...
def setup_diff_cotta(model):
    model = DiffusionCoTTA.configure_model(model)
    params, param_names = DiffusionCoTTA.collect_params(model)
    optimizer = setup_optimizer(params)
    cotta_model = DiffusionCoTTA(model, optimizer,
                        steps=cfg.OPTIM.STEPS,
                        episodic=cfg.MODEL.EPISODIC,
                        window_length=cfg.TEST.WINDOW_LENGTH,
                        dataset_name=cfg.CORRUPTION.DATASET,
