...
class CustomDatasetFolder(VisionDataset):
    ...
    def __init__(self, root, loader, extensions=None, transform=None, target_transform=None, is_valid_file=None, isDiffusion=False):
    ...
    def __getitem__(self, index):
        ...
        """
        if not self.isDiffusion:
            path, target = self.samples[index]
            domain, target = self.samples[index]
            ...
            path, target = self.samples[index]
            domain, target = self.samples[index]
            sample_d = self.loader(path)
            sample_c = self.loader(path.replace('generated_imagenet', 'imagenetc'))
            if self.transform is not None:
                sample_c = self.transform(sample_c)
                sample_d = self.transform(sample_d)
                # sample_c = transforms.Resize((224, 224))(sample_c)
                # sample_d = transforms.Resize((224, 224))(sample_d)
                # f, ax = plt.subplots(1, 2)
                # ax[0].imshow(sample_diifu.permute(1, 2, 0), label='diffu')
                # ax[1].imshow(sample_c.permute(1, 2, 0), label='c')
                # plt.savefig('testinging.png')
                sample_c = sample_c.unsqueeze(0)
                sample_d = sample_d.unsqueeze(0)
                sample = torch.cat((sample_c, sample_d), dim=0)
            if self.target_transform is not None:
                target = self.target_transform(target)
...
class CustomImageFolder(CustomDatasetFolder):
    ...
    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader, is_valid_file=None, isDiffusion=False):
...
class CustomCifarDataset(data.Dataset):
    def __init__(self, samples, transform=None, isDiffusion=False):
