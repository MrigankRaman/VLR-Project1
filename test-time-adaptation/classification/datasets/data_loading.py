...
from datasets.corruptions_datasetsdiffu import create_diffu_cifarc_dataset, create_diffu_imagenetc_dataset
...
def get_transform(dataset_name, adaptation):
    ...
    if adaptation == "adacontrast":
        ...
        elif dataset_name in {"cifar10_c", "cifar100_c", "cifar10_cd"}:
            transform = None
        elif dataset_name == "imagenet_c":
    ...
    if domain_name == "none":
        ...
        if dataset_name in {"cifar10_c", "cifar100_c"}:
            test_dataset = create_cifarc_dataset(dataset_name=dataset_name,
                                                 ...
                                                 transform=transform,
                                                 setting=setting)
        elif dataset_name in {'cifar10_cd'}:
            test_dataset = create_diffu_cifarc_dataset(dataset_name=dataset_name,
                                                       severity=severity,
                                                       data_dir=data_dir,
                                                       corruption=domain_name,
                                                       corruptions_seq=domain_names_all,
            ...
            test_dataset = create_imagenetc_dataset(n_examples=num_examples,
                                                    severity=severity,
                                                    data_dir='../../DDA/dataset/imagenetc',
                                                    corruption=domain_name,
                                                    corruptions_seq=domain_names_all,
                                                    transform=transform,
                                                    setting=setting)
        elif dataset_name == "imagenet_cd":
            test_dataset = create_diffu_imagenetc_dataset(n_examples=50000,
                                                            severity=severity,
                                                            data_dir='../../DDA/dataset/generated_imagenet',
                                                            corruption=domain_name,
                                                            corruptions_seq=domain_names_all,
                                                            transform=transform,
                                                            setting=setting)
